...
Function UpdateEvent_Cont_079(e.Events)
	Local e2.Events, it.Items
	
	If PlayerRoom = e\room Then
		
		If e\EventState = 0 Then
			
			e\room\NPC[0]=CreateNPC(NPCtypeGuard, EntityX(e\room\Objects[2],True)+0.25, EntityY(e\room\Objects[2],True)+0.5, EntityZ(e\room\Objects[2],True)-0.25)
			...
			it = CreateItem("USP Tactical", "usp",EntityX(e\room\Objects[2],True)+0.15,EntityY(e\room\Objects[2],True)+0.5,EntityZ(e\room\Objects[2],True)+0.35)
		...
		ShouldPlay = 4
		;This event will be heavily modified when we are able to. -Meow
	...
	If e\EventState2 = 1 Then
		If RemoteDoorOn Then
			...
			EndIf
			e\SoundCHN = StreamSound_Strict("SFX\SCP\079\GateB.ogg",opt\SFXVolume,0)
			e\SoundCHN_isStream = True
			e\EventState2 = 2
			
			For e2.Events = Each Events
				If e2\EventName="gate_b_entrance" Lor e2\EventName="gate_a_entrance" Then
					e2\EventState3 = 1
				EndIf
			Next
		EndIf	
