...
Function UpdateEvent_Room1_Sewers(e.Events)
	...
	If PlayerRoom = e\room
		If EntityY(Collider)<-3500.0*RoomScale ;Sewers Area
			...
			PositionTexture e\room\Objects[ROOM1_SEWERS_WATERTEXTURE_ID], 0, e\EventState3 * 0.002 ;0.005
		...
		If e\EventState2 < 1.0 Then
			For fb = Each FuseBox
				If fb\obj = e\room\Objects[ROOM1_SEWERS_FUSEBOX_ID] Then
					If (Not TaskExists(TASK_FINDFUSE)) And (Not TaskExists(TASK_FINDFUSEBOX)) And e\EventState2 = 0.0 And Msg = GetLocalString("Singleplayer", "fuse_find") Then
						...
						For i = 0 To MaxItemAmount-1
							If Inventory[i] <> Null Then
								If Inventory[i]\itemtemplate\tempname = "fuse" Then
									If (Not TaskExists(TASK_FINDFUSEBOX)) And e\EventState2 = 0.0 Then
		...
		If e\EventState >= 2.0 And e\EventState < 3.0 Then
			BlinkTimer = -10.0
			...
			ResetEntity Collider
			e\EventState = e\EventState + (FPSfactor / 70.0)
			user_camera_pitch = -89
			...
			EndIf
			DropSpeed = 0
		ElseIf e\EventState >= 4.0 And e\EventState < 6.0 Then
			e\EventState = e\EventState + (FPSfactor / 210.0)
			If e\EventState >= 5.0 Then
				user_camera_pitch = 89.0 * (e\EventState - 6.0)
				CrouchState = (6.0 - e\EventState)
				RotateEntity Camera, EntityPitch(Camera), EntityYaw(Camera), 20.0 - Abs(Min((5.5 - e\EventState) * 40.0, 20.0))
