...
Function UpdateNPCtype035MP(n.NPCs)
	...
	If (Not n\IsDead) Then
		Select n\State
			...
			Case MP035_STATE_ATTACK
				...
				If n\Frame => finalFrame Then
					...
					Else
						...
						EndIf
					EndIf
					If n\State = MP035_STATE_ATTACK Then
			...
			Case MP035_STATE_STUNNED
				...
				If prevFrame < 629.5 And n\Frame >= 629.5 Lor prevFrame = 630.0 And n\Frame = 630.0 Then
					...
					If n\State <> MP035_STATE_DETECTED Then
						n\State = MP035_STATE_WANDER
					EndIf
				EndIf
		...
		If n\State = MP035_STATE_WANDER Lor n\LastSeen <= 0 Then
			If n\CurrSpeed > 0.01 Then
		...
		If mp_I\PlayState = GAME_SERVER Then
			If n\State > MP035_STATE_FREEZE And n\State < MP035_STATE_ATTACK Then
				If Abs(n\DropSpeed) < 0.1 Then
					If n\State3 <= 0.0 Then
						If Rand(5) = 1 Then
							n2 = CreateNPC(NPCtypeTentacle, EntityX(n\Collider), EntityY(n\Collider) - (n\CollRadius/2.0) - 0.05 , EntityZ(n\Collider))
		...
		If n\PrevState = 0 Then
			If n\HP <= agitateHealth Then
				...
				SetAnimTime n\obj,0
				For i = 0 To 1
					n2 = CreateNPC(NPCtypeTentacle, 0, 0, 0)
					n2\Target = n
