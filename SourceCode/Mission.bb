...
Type Mission
	...
	Field startroom$
	Field musicname$
	Field InvDataName$[MaxItemAmount]
	Field InvDataTempName$[MaxItemAmount]
	Field TaskAmount%
	Field TaskText$[5]
	Field GunInHand%
...
Function InitMission(id%)
	...
	Select id
		Case 0
			...
			CreateTask(currmi\TaskText[1],1,"checkpoint1_new")
			
			currmi\InvDataName[0] = "USP Tactical"
			currmi\InvDataTempName[0] = "usp"
			currmi\InvDataName[1] = "Level 5 Key Card"
			...
			currmi\musicname = ""
			currmi\startroom = "start"
			
			currmi\GunInHand = 1
			NTF_CurrZone = 1
			currmi\Zone = NTF_CurrZone
			
			currmi\Spawn106 = True
			currmi\Spawn173 = True
			
			CreateMissionRoom(0, 0, "start", 180)
			CreateMissionRoom(0, -1, "room3")
			CreateMissionRoom(1, -1, "room2", 90)
		...
		Case 1
			...
			ta = CreateTask(currmi\TaskText[3],2,448,0,0,False,"room2elevator")
			ta\chaintask = CreateTask("",3,752,240,0,280,"room2elevator")
			CreateMissionEvent(0,448,0,0,False,"room2elevator")
			
			currmi\InvDataName[0] = "Silenced USP Tactical"
			currmi\InvDataTempName[0] = "usp"
			
			currmi\musicname = "Mission2Test"
			currmi\startroom = "checkpoint1_new"
	...
	For y = MapHeight - 1 To 1 Step - 1
		...
		For x = 1 To MapWidth - 2
			If MapTemp[x * MapWidth + y] > 0 Then
				If NTF_CurrZone = 2 Then temp=2 Else temp=0
...
Function InitMissionGameMode(id%)
	...
	Next
	For i = 1 To 10
		If currMission\GunUpgraded[i]=1
			For g = Each Guns
				If g\ID = i
					AddSilencer(g,False)
				EndIf
...
Function UpdateMissionEnding()
	...
	If KillTimer < 0
		If mi_I\EndingTimer = 0
			...
			If mi_I\EndSFX=0
				mi_I\EndSFX = StreamSound_Strict("SFX\Music\MissionModeFailed_Test.ogg",MusicVolume)
				mi_I\EndSFX_Vol = 1.0
			EndIf
