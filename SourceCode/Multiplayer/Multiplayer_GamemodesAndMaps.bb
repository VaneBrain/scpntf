...
Function CreateMPGameModes()
	...
	mgm\Image = LoadImage_Strict("GFX\menu\gamemode_preview_deathmatch.png")
	mgm\Image = ResizeImage2(mgm\Image, 384 * MenuScale, 192 * MenuScale)
							...
							Else	
								For i = 0 To (mp_I\MaxPlayers-1)
									If Players[i]<>Null Then
										...
										EndIf
										SetupTeam(i)
										Players[i]\WeaponInSlot[SLOT_PRIMARY] = GUN_UNARMED
										Players[i]\WeaponInSlot[SLOT_SECONDARY] = GUN_USP
										Players[i]\WeaponInSlot[SLOT_MELEE] = GUN_KNIFE
					...
					If EntityInView(fb\obj, Camera) Then
						...
						If PickedEntity() = fb\obj Then
							...
							If keyhituse Then
								...
								PlaySound_Strict PickSFX[Players[mp_I\PlayerID]\Item\itemtemplate\sound]
							...
							If keyhituse Then
								If SelectedItem <> Null Then
									If SelectedItem\itemtemplate\tempname = "fuse" Then
										...
										Msg = GetLocalString("Singleplayer", "fuse_placed")
									Else
										Msg = GetLocalString("Singleplayer", "fuse_cantplace")
									EndIf
								Else
									Msg = GetLocalString("Singleplayer", "fuse_find")
								EndIf
								MsgTimer = 70*5
							EndIf
						EndIf
					...
					EndIf	
						If mp_I\BossNPC <> Null Then
							If EntityDistanceSquared(mp_I\BossNPC\Collider, dbr\obj) < PowTwo(dbr\range) Then
								...
								If mp_I\BossNPC\State <> 3 Then
									mp_I\BossNPC\State = 3 ;Assume that all bosses would have State 3 as a Stunned state
								EndIf	
							EndIf
						EndIf
					EndIf
...
;~F#2A#3D#4C#5F#63#A3#C9#DD#109#15F#170#1CE#212#2A8#2E6#2F5#2FC#31F#368#372
;~F#38C#3BA#3C3#3CE#3DA#3E1#3EE#3FD#406#429#44E#459#48E#4B0#4CA#4D6#4E6#53C#543#550
;~F#588#59A#5A5
;~C#Blitz3D